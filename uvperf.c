 ...
 *
 *   Usage:
 *   uvperf -vVID -pPID -iINTERFACE -aAltInterface -eENDPOINT -mTRANSFERMODE -tTIMEOUT -lREADLENGTH
 *-wWRITELENGTH -rREPEAT -S1 -R|-W|-L
 *
 *   -vVID           USB Vendor ID
 *   -pPID           USB Product ID
...
int main(int argc, char **argv) {
    ...
    while (!TestParms.isCancelled) {
        ...
        if (ReadTest && ReadTest->LastTick.tv_nsec - ReadTest->StartTick.tv_nsec >= 60000.0) {
            TestParms.isUserAborted = TRUE;
            TestParms.isCancelled = TRUE;
            LOG_MSG("Elapsed Time %.2f  seconds\n",
        ...
        if (WriteTest && WriteTest->LastTick.tv_nsec - WriteTest->StartTick.tv_nsec >= 60000.0) {
            TestParms.isUserAborted = TRUE;
            TestParms.isCancelled = TRUE;
            LOG_MSG("Elapsed Time %.2f  seconds\n",
